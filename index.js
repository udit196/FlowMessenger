...
app.get('/chatbox', async (req, res) => {
  if(req.isAuthenticated()){
    try {
      ...
      res.render('chatbox', { user:req.session.user,user_name: req.session.user, friend: req.session.friend,friend_name, sendButton:'send-button.png', backButton:'back-button2.png',dp:"dp.png"});
      } catch (err) {
        console.error(err);
